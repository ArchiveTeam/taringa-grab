...
LUA_SHA1 = get_hash(os.path.join(CWD, 'taringa.lua'))
...
class WgetArgs(object):
    ...
    def realize(self, item):
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'story':
                wget_args.extend(['--warc-header', 'taringa-story: '+item_value])
                wget_args.append('https://api-beta.taringa.net/story/'+item_value)
            ...
            #elif item_type == 'user':
            #    wget_args.extend(['--warc-header', 'taringa-user: '+item_value])
            #    wget_args.append('https://api-beta.taringa.net/user/'+item_value+'/about')
            #elif item_type == 'comment':
            #    wget_args.extend(['--warc-header', 'taringa-comment: '+item_value])
            #    wget_args.append('https://api-beta.taringa.net/comment/'+item_value)
            #elif item_type == 'tag':
            #    wget_args.extend(['--warc-header', 'taringa-tag: '+item_value])
            #    wget_args.append('https://www.taringa.net/tags/'+item_value)
            else:
...
project = Project(
    title=TRACKER_ID,
    project_html='''
        <img class="project-logo" alt="Project logo" src="https://wiki.archiveteam.org/images/0/0b/Taringa_icon.png" height="50px" title=""/>
        <h2>Taringa! <span class="links"><a href="https://taringa.net/">Website</a> &middot; <a href="http://tracker.archiveteam.org/taringa/">Leaderboard</a> &middot; <a href="https://wiki.archiveteam.org/index.php/Taringa!">Wiki</a></span></h2>
        <p>Archiving Taringa!.</p>
    '''
)
